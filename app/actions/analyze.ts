...
16. **데이터테이블 설계**: 게임의 수치 데이터(캐릭터 스탯, 아이템 능력치, 몬스터 정보, 스킬 데이터 등)가 엑셀이나 표 형태의 데이터테이블로 설계되어 있는가. 어트리뷰트(컬럼) 설계가 게임플레이 의도를 반영하는가. 인스턴스(행) 간의 밸런스와 성장곡선이 적절한가. 새로운 콘텐츠를 추가할 때 테이블 구조를 바꾸지 않고 확장할 수 있는가. **데이터테이블이 문서에 전혀 없으면 0점을 주고, feedback에서 "게임 기획 포트폴리오에는 반드시 수치 데이터테이블이 포함되어야 합니다. 엑셀로 캐릭터/아이템/몬스터 등의 밸런스 테이블을 설계하여 포트폴리오에 첨부하세요."라고 안내하세요.**
...
## 응답 형식 (반드시 JSON만 출력, 다른 텍스트 없이)
{
  ...
  "categories": [
    ...
    { "subject": "데이터테이블", "value": 0, "fullMark": 100, "feedback": "[보완] 게임 기획 포트폴리오에는 반드시 수치 데이터테이블이 포함되어야 합니다. 엑셀로 캐릭터/아이템/몬스터 등의 밸런스 테이블을 설계하여 포트폴리오에 첨부하세요. 합격자들은 어트리뷰트 설계에 게임플레이 의도를 반영하고, 인스턴스 간 밸런스 곡선과 확장성까지 고려합니다." }
  ],
  ...
  "companyFeedback": "회사별 합격자 특징을 주어로 시작하여 비교. 예: '넥슨 합격자들은 수치 기반 시스템 설계가 탄탄하고, 크래프톤 합격자들은 전투 경험 목표가 구체적입니다. 이 문서는 ~가 부족하여 합격 수준에 도달하려면 ~를 보완해야 합니다.' 3~4문장으로. 절대 '~사례처럼', '~처럼' 표현 금지. 사용자는 합격자 포트폴리오를 볼 수 없으므로 '합격자들은 ~한 특징이 있다'는 식으로 서술."
    ...
    const message = await anthropic.messages.create({
      ...
      messages: [
        {
          role: "user",
          content: `아래는 웹 페이지(${input.url})에서 추출한 텍스트 내용입니다. 이 내용을 게임 기획 포트폴리오로서 분석해주세요. 시스템 프롬프트의 평가 기준과 합격 사례들을 참고하여 JSON 형식으로만 응답해주세요. 반드시 16개 categories와 companyFeedback을 모두 포함해야 합니다.\n\n---\n\n${pageContent}`,
        },
      ],
      system: systemPrompt,
    })
        ...
        messages: [
          {
            ...
            content: [
              ...
              {
                type: "text",
                text: "위 문서를 분석해주세요. 시스템 프롬프트의 평가 기준과 합격 사례들을 참고하여 JSON 형식으로만 응답해주세요. 반드시 16개 categories, companyFeedback, readabilityCategories(10개), layoutRecommendations(3개)를 모두 포함해야 합니다.",
              },
            ],
          },
        ],
