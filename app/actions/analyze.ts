...
### 전체 통계 — 주의: 아래 점수는 **합격자 표본**의 평균이며, 이미 합격한 문서들이라 평균이 높음. 사용자 문서가 이 점수에 못 미치는 것은 정상적이며, 그 경우 50~70점대로 평가하는 것이 적절함.
- 합격자 표본 평균: ${avgScores.overall}점
- 합격자 논리력 평균: ${avgScores.logic}점 | 구체성 평균: ${avgScores.specificity}점
- 합격자 가독성 평균: ${avgScores.readability}점 | 기술이해 평균: ${avgScores.technical}점
- 합격자 창의성 평균: ${avgScores.creativity}점
- 주요 키워드: ${topTags.join(", ")}
${deepStatsSection}
### 회사별 평균 점수
...
### 🚨 점수 척도 분리 (절대 혼동 금지!)
- 위에 표시된 **합격 사례들의 점수(78~95점대)는 "이미 합격한 표본"**이므로 자연스럽게 높은 점수에 몰려 있음.
- **현재 평가 대상 문서는 합격자가 아닐 가능성이 더 높음.** 따라서 점수 분포는 **0~100점 전체 범위**에서 자유롭게 결정해야 함.
- **합격 평균(${avgScores.overall}점)을 "기준선"으로 삼지 마라.** 그건 표본 편향이고 사용자 문서의 평가 척도가 아님.
- 사용자 문서가 합격자 수준에 명백히 미치지 못하면 **40~70점**도 정당하고 정상적임. 망설이지 말고 낮은 점수 부여.
- 거꾸로 사용자 문서가 합격자보다 명백히 우수하면 **90점대**도 정당함.
...
**최저점 78점 캡 절대 금지.** 합격자 표본의 최저점이 78이라는 사실이 사용자 문서의 하한선이 되어선 안 됨.
`
    }
