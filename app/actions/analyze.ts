...
## 응답 형식 (반드시 JSON만 출력, 다른 텍스트 없이)
{
  ...
  "strengths": [
    "강점1 - 합격자 평균과 비교한 구체적 강점 (예: 합격자 평균 대비 플로우차트 활용도가 뛰어남)",
    "강점2 - 구체적 수치 비교 (예: 합격자 평균 대비 120% 수준의 데이터 제시)",
    "강점3 - 합격 문서들의 공통 패턴과 일치하는 부분",
    "강점4 - 차별화 요소"
  ],
  "weaknesses": [
    "보완점1 - 합격자 평균 대비 부족한 점 (예: 합격 문서들은 평균 5개 다이어그램 사용, 현재 문서는 2개)",
    "보완점2 - 개선 방향 (예: 합격자들의 공통 패턴처럼 KPI 수치 명시 필요)",
    "보완점3 - 구조 개선",
    "보완점4 - 내용 보강"
  ]
}
...
**중요**:
1. "합격자 평균 대비...", "합격 문서들의 공통 패턴과 비교하면..." 등 **일반적인 비교 표현**을 사용하세요.
2. 절대로 "합격 사례 1번", "합격 사례 3번" 같은 **특정 번호를 언급하지 마세요**. 사용자는 학습 데이터를 볼 수 없습니다.
3. 대신 "합격자 평균", "상위 합격 문서들", "합격 문서들의 공통 패턴" 등의 표현을 사용하세요.
4. 학습 데이터를 기반으로 한 실질적이고 전문적인 피드백을 제공하세요.`
