...
`
      ...
      const topExamples = selectedExamples.map((p, idx) => {
        ...
        const deepInfo = hasDeepAnalysis
          ...
          : null
        // 합격자 = 100점 기준이므로 점수 라벨 노출 X. 패턴/특징만 인용.
        void deepInfo
        return `
### 합격 사례 ${idx + 1}: ${p.file_name} (100점 기준선)
- 지원사: ${(p.companies || []).join(", ")}
- 문서유형: ${p.document_type || "포트폴리오"}
- 핵심 강점:
${strengthsList}
${weaknessesList ? `- 보완 포인트(합격자 입장에서):
${weaknessesList}` : ""}
- 요약: ${p.summary || "N/A"}
`
      }).join("\n")
      ...
      const companyPatterns = Object.entries(companyStats)
        ...
        .map(([company]) => {
          ...
          return `- **${company}** (${companyPortfolios.length}개 합격 사례, 100점 기준선)
  ...
  · 주요 키워드: ${topCompanyTags.join(", ") || "데이터 부족"}`
      ...
      referenceStats = `
## 📊 학습 데이터 — 합격 포트폴리오 ${portfolios.length}개 (참고용)

### 🎯 절대 기준: 아래 합격 사례들은 **모두 100점 만점 기준선**으로 간주한다.
사용자 문서가 이 합격 사례들 수준에 비해 **얼마나 부족한지**를 평가하라.
합격 표본의 점수 분포는 표시하지 않는다 (그 분포에 사용자 문서가 끌려가면 안 되므로).
...
${Object.entries(companyStats).slice(0, 8).map(([company, stat]) =>
  `- ${company}: ${stat.count}개 합격 사례 (모두 100점 기준선)`
).join("\n")}
...
## ⚠️ 평가 절차 — 합격자 = 100점 기준선

### 절차
1. 위 합격 사례들의 **공통 패턴, 강점 요소, 문서 구조**를 머리에 그린다 (이게 100점 수준).
2. 현재 사용자 문서를 검토한다.
3. 사용자 문서가 합격자 수준에 비해 어떤 요소가 얼마나 부족한지 따진다.
4. 부족한 정도를 점수로 환산: 합격자 100점 → 거의 같다면 90점 → 다소 부족하면 75점 → 핵심 요소 다수 누락이면 55점.

### 비교 점검 항목
1. 합격 사례들이 공통적으로 가진 강점을 현재 문서도 갖추었는가? (없으면 60점대 이하)
2. 합격 사례들과 비교해 어느 정도 격차가 있는가? (격차가 클수록 점수 낮게)
3. 같은 회사 지원 사례가 있다면, 그 수준에 얼마나 근접한가?

### 핵심 금지 사항
- **합격자 표본의 점수 분포를 추정해서 그 안에 사용자 문서를 욱여넣지 마라.** 합격자는 모두 100점이라 가정.
- **"안전한 중간값"인 70점대 후반(75, 78)에 수렴하지 마라.** 합격자에 가깝다면 85점+, 부족하면 60점대 이하로 과감하게.
- 일반 사용자 문서의 가장 흔한 점수대는 **50~75점**임을 기억하라.
`
    }
...
## 🎯 점수 채점 기준 — "합격자 = 100점" 절대 기준 (가장 중요!)

### 절대 원칙
**모든 합격 사례(위 ${portfolios?.length || 0}개 표본)는 100점 만점 기준선이다.**
사용자 문서가 합격자 수준에 비해 **얼마나 부족한가**로 점수를 매긴다.
합격자 표본의 실제 점수 분포는 무시한다 (그건 표본 편향이며 사용자 평가 척도가 아님).

### 구간별 의미 (절대 척도)
- 95-100점: 합격자 사례와 동급 또는 우수
- 85-94점: 합격자에 매우 가까움, 일부 보완만 필요
- 70-84점: 합격자 수준에 미흡, 다수 보완 필요 (개선 시 합격 가능)
- 50-69점: 핵심 요소 다수 누락 (상당한 보완 필요) — **가장 흔한 일반 사용자 점수대**
- 30-49점: 기획서/포트폴리오로서 미완성
- 0-29점: 빈약하거나 주제 불일치

### 분포 가이드 (모델이 가져야 할 직관)
- 일반 사용자는 합격자가 아니므로 **50~75점이 가장 흔한 점수대**
- 합격급 사용자(드뭄)는 80~95점
- 명백히 부족한 문서(흔함)는 40~65점
- 점수가 항상 70점대 후반에 몰리면 표본 편향에 빠진 것이므로 즉시 재평가

### ⚠️ 점수 차별화 필수 규칙
1. **15개 categories의 value를 모두 다른 값으로 매겨라.** 같은 점수가 3개 이상 나오면 안 됨.
2. **70점대 후반(75~79)으로 수렴하지 마라.** 78, 75, 76 같은 "안전한 중간값" 남발 금지.
3. **흔한 정수(70, 75, 78, 80)를 회피하고 53, 67, 73, 84, 91 같은 다양한 숫자 사용.**
4. **각 카테고리도 "합격자 = 100점" 동일 기준 적용.**
   - 합격자가 보유한 요소를 100% 갖췄다 → 100점
   - 70%만 갖췄다 → 70점
   - 거의 없다 → 30~40점
5. **전체 score는 15개 categories의 가중평균. 절대 70점대로 자동 수렴시키지 말 것.**

### 검증 체크 (점수 부여 후 스스로 확인!)
- [ ] 모든 점수가 70~80 사이에 몰려있지 않은가? (몰려있으면 표본 편향)
- [ ] 사용자 문서가 합격자에 비해 명백히 부족한데 점수가 75점 이상인가? (그러면 박하게 다시 부여)
- [ ] 78점이 두 번 이상 나왔는가? (나왔으면 다른 점수로 변경)
...
{
  "score": 63,  /* 합격자 100점 기준에서 사용자 문서가 부족한 만큼 차감한 점수. 일반 사용자는 50~75점이 흔함 */
  "categories": [
    ...
    { "subject": "가독성", "value": 64, "fullMark": 100, "feedback": "[강점] 잘 된 부분.\\n[보완] 부족한 점. 3줄 이상." },
    { "subject": "기술이해", "value": 67, "fullMark": 100, "feedback": "[강점] 잘 된 부분.\\n[보완] 부족한 점. 3줄 이상." },
...
5. **합격 평균(${avgScores.overall}점)은 합격자 표본의 평균일 뿐, 사용자 문서의 "기본 점수"가 아니다.**
   - 사용자 문서가 합격자 수준에 미달하면 **50~75점**도 정당함 (가장 흔한 케이스)
   - 합격자 평균 근처(±5점)는 명확히 합격자급 수준일 때만 부여
   - 합격자 표본 최저점(약 78점)을 사용자 문서의 하한선으로 삼지 말 것 — **사용자 문서는 30점도 가능, 95점도 가능**
...
## 응답 형식 (반드시 JSON만 출력, 다른 텍스트 없이)
{
  ...
  "readabilityCategories": [
    { "subject": "글자크기구분", "value": 67, "fullMark": 100, "feedback": "벤치마크 참고하여 합격자 대비 제목/본문 크기 구분에 대한 구체적 피드백" },
    ...
    { "subject": "읽는순서", "value": 71, "fullMark": 100, "feedback": "벤치마크 참고하여 합격자 대비 읽는 순서 피드백" },
