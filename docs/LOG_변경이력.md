...
- `app/payment/credits/page.tsx`: NICEPay `method: "card"` → `"cardAndEasyPay"`로 변경
- 카드 결제와 함께 네이버페이, 카카오페이 등 간편결제 수단이 결제창에 노출됨
- 카드사 심사 승인 완료 후 적용

### 크레딧 차감 모달 'N회' → 'N크레딧' 문구 수정 (`a14b4ff`)
- `components/analyze-dashboard.tsx`: 크레딧 차감 확인 모달 + 무료 플랜 배너의 '회' 단위를 '크레딧'으로 변경
- NICEPay 카드사 심사 대응

### 전체 '회권/회차권' → '크레딧' 문구 변경 (`acd6f57`)
- 소스 7개 파일 + docs 9개 파일에서 '회권', '회차권' 용어를 '크레딧'으로 일괄 변경
- NICEPay 카드사 심사 요구사항 대응
