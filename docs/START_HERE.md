...
11년차 현업 게임 기획자(강사)가 만든 **포트폴리오 AI 피드백 + 면접 연습 + 게임 디자인 라이브러리** 통합 서비스.
취준생이 포트폴리오를 업로드하면 Claude AI가 점수/랭킹/피드백을 제공하고, 분석 중 자동으로 게임 디자인 라이브러리(1,243편)의 원칙·패턴을 인용한다.
...
- ✅ **게임 디자인 라이브러리 통합** (2026-05-13) — Obsidian Vault 998+ .md → 1,243 문서 임포트, /library/* 라우트, 그래프 뷰(react-force-graph-2d), analyze.ts에 라이브러리 청크 자동 인용
...
| `PRD_라이브러리.md` | **신규** — 게임 디자인 라이브러리 통합 (Vault → DB, 라우트, 그래프, analyze 연동) |
